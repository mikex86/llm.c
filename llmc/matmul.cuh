...
void matmul_triton(floatX *d, const floatX *a, const floatX *b, const floatX *bias,
                   ...
                   int batch_count = 0, size_t strideA = 0, size_t strideB = 0, size_t strideOut = 0,
                   bool accumulate = false, floatX *pre_gelu = nullptr, bool backward = true) {
...
bool triton_matmul_lp_accum = false;

// small wrapper around matmul_cublaslt for the forward pass (keeping historical order of arguments)
void matmul_forward(floatX* out,
                     floatX* inp, floatX* weight, floatX* bias,
                     int B, int T, int C, int OC, cudaStream_t stream,
                     floatX* pre_gelu=NULL, int gelu_fusion=1, bool can_use_lp_accumulator=true) {
    ...
    if (gelu_fusion < 1 && pre_gelu) {
        if (PRECISION_MODE == PRECISION_FP16 && triton_matmul_lp_accum && can_use_lp_accumulator) {
